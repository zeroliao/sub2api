...
const BulkEditAccountModalStub = {
  props: ["show", "target"],
  template:
    '<div data-test="bulk-edit-modal" :data-show="String(show)" :data-target-mode="target?.mode ?? \'\'"></div>',
};
...
describe("admin AccountsView bulk edit scope", () => {
  ...
  it("opens bulk edit in filtered-results mode from the bulk actions dropdown", async () => {
    ...
    expect(
      wrapper.get('[data-test="bulk-edit-modal"]').attributes("data-show"),
    ).toBe("true");
    expect(
      wrapper
        .get('[data-test="bulk-edit-modal"]')
        .attributes("data-target-mode"),
    ).toBe("filtered");
  });
  ...
  it("passes the loaded global probe state to every upstream billing cell", async () => {
      ...
      global: {
        stubs: {
          ...
          DataTable: {
            props: ["data"],
            template:
              '<div><div v-for="row in data" :key="row.id"><slot name="cell-upstream_billing_rate" :row="row" /></div></div>',
          },
          UpstreamBillingRateCell: {
            props: ["globalProbeEnabled"],
            template:
              '<span data-test="upstream-billing-cell" :data-global-enabled="String(globalProbeEnabled)"></span>',
          },
          Pagination: true,
          ConfirmDialog: true,
    ...
    expect(
      wrapper
        .get('[data-test="upstream-billing-cell"]')
        .attributes("data-global-enabled"),
    ).toBe("false");
  });
    ...
    const wrapper = mount(AccountsView, {
      global: {
        stubs: {
          ...
          TablePageLayout: {
            template:
              '<div><slot name="table" /><slot name="pagination" /></div>',
          },
          DataTable: DataTableStub,
  ...
  it("reloads the server-sorted list after a batch probe changes a snapshot", async () => {
    localStorage.setItem(
      "account-table-sort",
      JSON.stringify({ key: "upstream_billing_rate", order: "asc" }),
    );
    const account = (id: number) => ({
      id,
      name: `account-${id}`,
