export default {
  accounts: {
    ...
    upstreamBilling: {
      ...
      autoProbeHint:
        "启用后按全局探测周期查询此账号的上游声明倍率；全局探测关闭时不会执行。",
    ...
    fillRecommendedModels: "填入推荐模型",
    fetchAPIKeyModels: "获取当前 Key 可用模型",
    apiKeyRequiredForModelSync: "请先填写 API Key，再获取可用模型",
    failedToFetchAPIKeyModels: "获取可用模型失败",
    noAPIKeyModelsReturned: "上游没有返回可用模型",
    apiKeyModelsSynced: "已获取 {count} 个可用模型",
