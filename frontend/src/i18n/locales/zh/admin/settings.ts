export default {
  settings: {
    ...
    features: {
      ...
      affiliate: {
        ...
        adminRechargeRebateHint:
          "开启后，通过“用户管理 > 充值”增加的余额会产生邀请返利；设置余额和扣款不参与返利。",
        rebateRate: "全局返利比例",
        rebateRateHint:
    ...
    security: {
      ...
      stepUpHint:
        "开启后，账号/代理导出、备份创建与下载、S3 配置修改、提升管理员等敏感操作需要先完成 TOTP 二次验证（15 分钟内有效）。开启前需本人已启用 2FA；关闭该开关本身也需要二次验证。",
      stepUpEnableRequiresTotp:
        "开启敏感操作二次验证前，请先在个人资料中为当前账号启用 2FA (TOTP)。",
      sessionBinding: "会话 IP/UA 绑定",
      sessionBindingHint:
        "将登录会话与客户端 IP 和 User-Agent 绑定，任一变化即强制该会话失效并需重新登录（提升被盗凭证的利用门槛）。",
    ...
    apiKeyAcl: {
      ...
      description:
        "控制 API Key 白/黑名单、操作审计日志与会话 IP/UA 绑定使用哪个客户端 IP 判断",
    ...
    dingtalk: {
      ...
      redirectUrlHint:
        "需与钉钉开放平台中配置的回调地址一致（必须是 http(s) 完整 URL）",
      corpPolicy: {
        label: "企业限制策略",
      ...
      syncDisplayNameHint:
        "登录时将钉钉姓名写入 username 字段（同时记录到 dingtalk_name 属性）",
      syncCorpEmail: "同步企业邮箱",
      syncCorpEmailHint:
        "登录时将钉钉企业邮箱写入 dingtalk_email 属性（不影响登录邮箱）",
      syncCorpEmailPermissionHint:
        "需在钉钉开放平台 → 应用 → 权限管理中为本应用申请「邮箱等个人信息（fieldEmail）」权限，否则 OAPI 不会返回企业邮箱字段",
      syncDept: "同步部门",
      syncDeptHint:
        "登录时将钉钉首个部门完整路径写入 dingtalk_department 属性（每次登录实时拉取）",
      syncDeptPermissionHint:
        "需在钉钉开放平台 → 应用 → 权限管理中为本应用申请「通讯录部门信息读权限（qyapi_get_department_list）」，否则无法递归出部门路径",
      syncDisplayNameTarget: "属性键",
      syncDisplayNameTargetHint:
        "默认 dingtalk_name / 钉钉姓名；保存设置时按上述属性键和显示名称自动创建用户属性（已存在则仅同步显示名称）",
      syncCorpEmailTarget: "属性键",
      syncCorpEmailTargetHint:
        "默认 dingtalk_email / 钉钉企业邮箱；保存设置时按上述属性键和显示名称自动创建用户属性（已存在则仅同步显示名称）",
      syncDeptTarget: "属性键",
      syncDeptTargetHint:
        "默认 dingtalk_department / 钉钉部门；保存设置时按上述属性键和显示名称自动创建用户属性（已存在则仅同步显示名称）",
    ...
    upstreamBillingProbe: {
      ...
      enabled: "启用全局自动探测",
      enabledHint:
        "开启后，仅对账号自身已启用自动检测的账号执行定时探测；关闭后停止所有定时探测，手动探测不受影响。",
    ...
    gatewayForwarding: {
      ...
      codexFpMatchPlaceholder:
        "匹配，变体用 / 分隔（如 session-id / session_id 或 x-codex-）",
      codexFpRequired: "必须",
      ...
      codexUaContainsPlaceholder:
        "User-Agent 包含标记，逗号分隔（如 opencode/）",
      codexAddRow: "添加一条",
      codexRemoveRow: "删除",
    },
    webSearchEmulation: {
    ...
    site: {
      ...
      apiBaseUrl: "API 端点地址",
      apiBaseUrlHint:
        '用于"使用密钥"、"导入到 CC Switch"和回调地址建议，留空则使用当前站点地址',
    ...
    payment: {
      ...
      alipayForceQRCodeHint:
        "启用后，移动端支付宝用户将统一使用二维码扫码支付，不再跳转至手机网站支付",
      helpText: "帮助文本",
      helpImageUrl: "帮助图片链接",
      ...
      easypayCustomMethodsHint:
        "添加当前易支付服务商额外支持的支付方式。支付方式会记录到 Sub2API 订单中，上游 type 会作为易支付 type 参数提交。",
    ...
    subscriptionExpiryNotify: {
      ...
      enabled: "启用订阅到期提醒",
      enabledHint:
        "开启后，系统会在订阅到期前 7 天、3 天、1 天各发送一次提醒。",
    },
    smtp: {
      title: "SMTP 设置",
    ...
    emailTemplates: {
      ...
      livePreview: "实时预览",
      previewSecurityHint:
        "预览 HTML 由后端预览接口生成，并在禁用脚本的沙盒 iframe 中展示。",
      preview: "预览 / 刷新",
      previewing: "预览中...",
      save: "保存模板",
      saving: "保存中...",
