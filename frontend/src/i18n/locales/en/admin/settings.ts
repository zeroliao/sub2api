  ...
  settings: {
    ...
    features: {
      ...
      affiliate: {
        ...
        adminRechargeRebateHint:
          "When enabled, balance added through User Management > Deposit generates affiliate rebates. Setting a balance or withdrawing funds does not.",
    ...
    security: {
      ...
      stepUpHint:
        "When enabled, sensitive operations (account/proxy export, backup creation and download, S3 config changes, promoting admins) require a recent TOTP verification (valid for 15 minutes). Your own account must have 2FA enabled before turning this on; turning it off also requires step-up verification.",
      stepUpEnableRequiresTotp:
        "Enable 2FA (TOTP) for your own account in Profile before turning on step-up verification.",
      sessionBinding: "Session IP/UA Binding",
      sessionBindingHint:
        "Bind login sessions to the client IP and User-Agent. Any change immediately invalidates the session and forces re-login, raising the bar for stolen-credential reuse.",
    ...
    apiKeyAcl: {
      ...
      forwardedClientIpHeadersHint:
        "Add CDN or proxy header names to check before the built-in headers.",
      forwardedClientIpHeadersPlaceholder: "X-Client-IP",
      forwardedClientIpHeadersRiskHint:
        "These raw headers can be spoofed when the origin is reachable directly. Restrict origin access before trusting them.",
      forwardedClientIpHeaderInvalid: "Enter a valid HTTP header name.",
      forwardedClientIpHeadersLimit:
        "At most {max} custom client-IP headers are allowed.",
    ...
    dingtalk: {
      ...
      redirectUrl: "Redirect URL",
      redirectUrlPlaceholder:
        "https://your-domain.com/api/v1/auth/oauth/dingtalk/callback",
      redirectUrlHint:
      ...
      bypassRegistrationHint:
        "Allow new users to register via DingTalk even when public registration is disabled.",
      syncDisplayName: "Sync DingTalk display name",
      syncDisplayNameHint:
        "Overwrite username with the DingTalk staff name on each login (also stored in the dingtalk_name attribute).",
      syncCorpEmail: "Sync corporate email",
      syncCorpEmailHint:
        "Write the DingTalk corporate email to the dingtalk_email attribute on each login (does not change the login email).",
      syncCorpEmailPermissionHint:
        'Requires the OAPI permission "Personal info incl. email (fieldEmail)" to be granted to the app on the DingTalk open platform, otherwise OAPI will not return the email field.',
      syncDept: "Sync department",
      syncDeptHint:
        "Write the full DingTalk department path to the dingtalk_department attribute on each login (fetched live each time).",
      syncDeptPermissionHint:
        'Requires the OAPI "Department info read (qyapi_get_department_list)" permission to be granted to the app on the DingTalk open platform, otherwise the department path cannot be resolved.',
      syncDisplayNameTarget: "Attribute key",
      syncDisplayNameTargetHint:
        "Defaults to dingtalk_name / DingTalk Name. Saving settings auto-creates the user attribute by the key and display name above (existing definition only has its display name synced).",
      syncCorpEmailTarget: "Attribute key",
      syncCorpEmailTargetHint:
        "Defaults to dingtalk_email / DingTalk Corporate Email. Saving settings auto-creates the user attribute by the key and display name above (existing definition only has its display name synced).",
      syncDeptTarget: "Attribute key",
      syncDeptTargetHint:
        "Defaults to dingtalk_department / DingTalk Department. Saving settings auto-creates the user attribute by the key and display name above (existing definition only has its display name synced).",
    ...
    defaults: {
      ...
      defaultPlatformQuotasHint:
        "Automatically assigned to new users on signup; existing users are not affected. Leave blank = unlimited.",
      platformQuotaNotice:
        "Monthly quota uses a 30-day rolling window, not a calendar month.",
    },
    platformQuota: {
      platform: "Platform",
      daily: "Daily (USD)",
    ...
    upstreamBillingProbe: {
      ...
      description:
        "Periodically retrieve billing rates declared by upstream Sub2API sites connected to OpenAI API keys.",
      enabled: "Enable global auto detection",
      enabledHint:
        "When enabled, scheduled detection runs only for accounts that also enable automatic detection. Disabling stops all scheduled detection; manual detection remains available.",
      intervalMinutes: "Detection interval (minutes)",
      intervalHint:
        "Range: 5–1440 minutes. A successful result remains valid for two detection intervals.",
    ...
    gatewayForwarding: {
      ...
      codexFpMatchPlaceholder:
        "match; '/'-separate variants (e.g. session-id / session_id or x-codex-)",
      ...
      codexUaContainsPlaceholder:
        "User-Agent contains markers, comma-separated (e.g. opencode/)",
    ...
    payment: {
      ...
      alipayForceQRCodeHint:
        "When enabled, mobile Alipay users always see a QR code instead of being redirected to the mobile payment page",
      helpText: "Help Text",
      ...
      validationEasyPayCustomMethodRequired:
        "Each custom EasyPay method requires both a payment type and an upstream type",
      validationEasyPayCustomMethodTypeInvalid:
        "Custom EasyPay payment types may only contain lowercase letters, digits, underscores, and hyphens",
      validationEasyPayCustomMethodUpstreamTypeInvalid:
        "EasyPay upstream types may only contain lowercase letters, digits, underscores, and hyphens",
      validationEasyPayCustomMethodReserved:
        "Custom EasyPay payment types cannot use built-in alipay or wxpay",
      validationEasyPayCustomMethodPrefixReserved:
        "Custom EasyPay payment types cannot start with alipay or wxpay",
      validationEasyPayCustomMethodDuplicate:
        "Custom EasyPay payment types must be unique",
      ...
      easypayCustomMethodsHint:
        "Add provider-specific methods supported by this EasyPay endpoint. The payment type is stored on Sub2API orders; the upstream type is sent as EasyPay type.",
    ...
    subscriptionExpiryNotify: {
      ...
      description:
        "Control whether users receive subscription expiry reminder emails.",
      enabled: "Enable Subscription Expiry Reminder",
      enabledHint:
        "When enabled, the system sends reminders 7, 3, and 1 day before expiry.",
    },
    smtp: {
      title: "SMTP Settings",
    ...
    emailTemplates: {
      ...
      description:
        "Customize notification email subjects and HTML content for each event and locale.",
      event: "Event",
      locale: "Locale",
      localeEn: "English",
      localeZh: "Chinese",
      ...
      placeholdersHelp:
        "Click a placeholder to copy it. The backend replaces these values when sending emails.",
      livePreview: "Live Preview",
      previewSecurityHint:
        "Preview HTML is generated by the backend preview endpoint and displayed in a sandboxed iframe with scripts disabled.",
      ...
      restoring: "Restoring...",
      restoreConfirm:
        "Restore the official template for this event and locale? Your custom version will be replaced.",
    ...
    authSourceDefaults: {
      ...
      platformQuotasOverrideHint:
        "Blank fields inherit the system default. Set to 0 to fully block that window for this auth source.",
    },
    ...
    openaiExperimentalScheduler: {
      ...
      lowRatePriorityDescription:
        "When enabled, accounts with lower billing rates are preferred. If rates are equal, account priority, current load, and other scheduling factors are considered. This switch is ignored when the experimental scheduler is enabled.",
      oauthRateTitle: "OAuth scheduling reference rate",
      oauthRatePriorityDescription:
        "When a group contains both API Key and OAuth accounts, this rate is used to order OAuth accounts alongside probed API Key billing rates.",
      oauthRateWeightedDescription:
        "When a group contains both API Key and OAuth accounts, this rate is used for OAuth accounts when calculating the billing-rate score.",
      stickyWeightedTitle: "Sticky weighting",
      stickyWeightedDescription:
        "When enabled, previous_response_id and session_hash affinity are scored by the advanced scheduler. When disabled, sticky accounts keep the legacy hard-hit behavior.",
      subscriptionPriorityTitle: "Subscription priority",
      subscriptionPriorityDescription:
        "When enabled, the scheduler scores ChatGPT subscription accounts first and falls back to non-subscription accounts only if no subscription slot can be acquired.",
      weightsTitle: "Scheduler weight overrides",
      weightsDescription:
        "Blank values use config/environment values; when config is not set, built-in defaults apply. Non-blank page settings take priority.",
    ...
    usageRecords: {
      title: "Usage Records",
      description:
        "Settings for usage and failed-request records visible to end users.",
    },
    user_error_view: {
