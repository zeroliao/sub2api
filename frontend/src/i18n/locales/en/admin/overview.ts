export default {
  ...
  dashboard: {
    ...
    startUsingApi:
      "Once you start using the API, your usage history will appear here.",
  ...
  backup: {
    ...
    imageStorage: {
      ...
      description:
        "Enables the asynchronous image endpoints and offloads generated images to object storage, keeping only short links in Redis. Shares the S3 client with backups and takes effect on save — no restart needed.",
      enabled: "Enable async image tasks",
      reuseBackupS3:
        "Reuse the backup S3 configuration above (different bucket/prefix only)",
      bucket: "Bucket",
  ...
  users: {
    ...
    platformQuota: {
      ...
      subtitle:
        "Configure daily / weekly / monthly USD usage limits for each upstream platform for user {email}",
      columns: {
        platform: "Platform",
        daily: "Daily (USD)",
        weekly: "Weekly (USD)",
      ...
      clearAllConfirm:
        'Clear daily / weekly / monthly limits for ALL platforms? All platforms will become "unlimited" with no local undo — you must manually re-enter values before saving.',
      reset: {
        button: "Reset window",
        confirm:
          "Reset the {window} usage for {platform} for this user? This is effective immediately.",
      ...
      subscriptionWarning:
        "This user has an active subscription. Platform quotas only apply to balance (standard) mode requests; subscription mode requests are not subject to these limits.",
      invalidNumber:
        "The following fields contain invalid numbers. Please fix them before saving: {fields}",
    },
  },
  ...
  groups: {
    ...
    duplicateSuccess:
      'Group duplicated as "{name}" and disabled. Review its configuration before enabling it.',
    ...
    accountFilters: {
      ...
      privacySetOnlyEnabled:
        "Enabled — accounts with unset Privacy will be excluded",
      disabled: "Disabled",
    },
    form: {
      name: "Name",
      ...
      maxReasoningEffortHint:
        "Limits explicit OpenAI reasoning effort requests only. Higher values are capped; omitted effort stays omitted. The ceiling takes precedence over reasoning effort mappings.",
      ...
      priorityHint:
        "Lower value means higher priority, used for account scheduling",
      statusLabel: "Status",
    },
    exclusiveObj: {
      yes: "Yes",
    ...
    imagePricing: {
      ...
      batchSectionHint:
        "Batch image settings only apply to batch jobs: settlement applies the batch discount, and the upfront hold is normal image price × batch hold price ratio. Reference images also create upstream input-token usage, so a batch image discount above 0.5 is recommended.",
      batchDisabledHint:
        "Enable image generation for this group before enabling batch image generation.",
      batchGeminiOnlyHint:
        "Batch image generation is currently available only for Gemini groups.",
      modeHint:
    ...
    peakRate: {
      ...
      multiplierHint:
        "Applies to token billing multiplier; image tokens in token billing are also affected. 0 means peak token requests are billed at 0x.",
    },
    modelsList: {
