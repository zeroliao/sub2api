...
const { updateAccountMock, checkMixedChannelRiskMock, authIsSimpleMode } =
  vi.hoisted(() => ({
...
describe("EditAccountModal Grok OAuth upstream config", () => {
  ...
  it("accepts the official API host as a manual endpoint switch and persists it", async () => {
    ...
    await wrapper
      .get('[data-testid="grok-custom-base-url-toggle"]')
      .trigger("click");
    await wrapper
      .get('[data-testid="grok-custom-base-url-input"]')
      .setValue("https://api.x.ai/v1");
  ...
  it("shows the client-tool cache switch only for Grok OAuth accounts", () => {
    ...
    expect(
      grokOAuthWrapper
        .find('[data-testid="grok-client-tool-cache-toggle"]')
        .exists(),
    ).toBe(true);
  ...
  it("shows malformed cache settings as disabled so the UI matches the fail-closed backend", () => {
    ...
    expect(
      wrapper
        .get('[data-testid="grok-client-tool-cache-toggle"]')
