...
vi.mock("@/api/admin", () => ({
  adminAPI: {
    ...
    settings: {
      getWebSearchEmulationConfig: vi
        .fn()
        .mockResolvedValue({ enabled: false, providers: [] }),
...
async function selectButtonByText(
...
async function submitApiKeyAccount(
  ...
  if (enableLongContextBilling) {
    await wrapper
      .get('[data-testid="openai-long-context-billing-toggle"]')
      .trigger("click");
  }
  if (disableUpstreamBillingProbe) {
    await wrapper
      .get('[data-testid="upstream-billing-auto-probe"]')
      .trigger("click");
  }
...
describe("CreateAccountModal OpenAI long-context billing", () => {
  ...
  it("sends false explicitly for normal OpenAI account creation by default", async () => {
    ...
    expect(
      createAccountMock.mock.calls[0]?.[0]?.extra
  ...
  it("enables upstream billing probes by default for new OpenAI API key accounts", async () => {
    ...
    expect(
      createAccountMock.mock.calls[0]?.[0]?.upstream_billing_probe_enabled,
    ).toBe(true);
  });
  ...
  it.each([
    [
      "camelCase",
      {
        ...
        agentIdentity: { agentRuntimeId: "runtime" },
      },
    ],
    [
      "nested identity without auth_mode",
      { agent_identity: { agent_runtime_id: "runtime" } },
    ],
  ])(
    "accepts backend-compatible %s Agent Identity imports",
    async (_name, content) => {
  ...
  it("leaves Codex session import billing ownership to the backend", async () => {
    ...
    expect(importCodexSessionMock).toHaveBeenCalledTimes(1);
    expect(
      importCodexSessionMock.mock.calls[0]?.[0]?.extra
        ?.openai_long_context_billing_enabled,
    ).toBeUndefined();
  });
  ...
  it("leaves Codex PAT import billing ownership to the backend", async () => {
    ...
    expect(
      createOpenAICodexPATMock.mock.calls[0]?.[0]?.extra
        ?.openai_long_context_billing_enabled,
    ).toBeUndefined();
  });
