...
describe("AccountUsageCell", () => {
      ...
      global: {
        stubs: {
          UsageProgressBar: {
            ...
            template:
              '<div class="usage-bar">{{ label }}|{{ utilization }}|{{ resetsAt }}</div>',
          },
  ...
  it("Key 账号会展示 today stats 徽章并带 A/U 提示", async () => {
    ...
    expect(
      badges.some((node) => node.attributes("title") === "usage.accountBilled"),
    ).toBe(true);
    expect(
      badges.some((node) => node.attributes("title") === "usage.userBilled"),
    ).toBe(true);
  });
  ...
  it("Grok OAuth 会展示本地 user billed 用量并把耗尽配额显示为 0% 剩余", async () => {
    ...
    expect(wrapper.text()).toContain(
      "admin.accounts.usageWindow.grokRequests|0|2026-07-09T16:00:00Z",
    );
  ...
  it("Grok OAuth 配额条按剩余容量显示 100% 满格和 25% 低量", async () => {
      ...
      global: {
        stubs: {
          UsageProgressBar: {
            ...
            template:
              '<div class="usage-bar">{{ label }}|{{ utilization }}|{{ remainingCapacity }}</div>',
          },
          AccountQuotaInfo: true,
          GrokQuotaProbeCell: true,
        },
    ...
    expect(wrapper.text()).toContain(
      "admin.accounts.usageWindow.grokRequests|100|true",
    );
    expect(wrapper.text()).toContain(
      "admin.accounts.usageWindow.grokTokens|25|true",
    );
  });
  ...
  it("Grok OAuth uses the official weekly billing percentage when available", async () => {
    ...
    expect(wrapper.text()).not.toContain(
      "admin.accounts.usageWindow.grokRequests|",
    );
    expect(wrapper.text()).not.toContain(
      "admin.accounts.usageWindow.grokTokens|",
    );
  ...
  it.each([
    ...
    "Grok Free derives its 1M quota from local tokens: $tokens -> $expected%",
    async ({ tokens, expected, compact }) => {
      getUsage.mockResolvedValue({
      ...
      expect(
        wrapper.findAll("span").filter((node) => node.text() === compact),
      ).toHaveLength(1);
  ...
  it("Grok Free uses rolling 24h usage instead of today-only usage", async () => {
      ...
      global: {
        stubs: {
          UsageProgressBar: {
            ...
            template:
              '<div class="usage-bar">{{ label }}|{{ utilization }}|{{ title }}</div>',
          },
          AccountQuotaInfo: true,
          GrokQuotaProbeCell: true,
        },
    ...
    expect(wrapper.text()).toContain(
      "24h|75|admin.accounts.usageWindow.grokFreeQuota24hHint",
    );
  ...
  it("Grok paid plans are not mistaken for Free when weekly usage is temporarily missing", async () => {
    ...
    expect(wrapper.text()).toContain(
      "admin.accounts.usageWindow.grokTokens|25",
    );
    expect(wrapper.text()).not.toContain("2M|");
  });
