...
describe("UpstreamBillingRateCell", () => {
  ...
  it("recomputes the current effective rate and keeps the icon-only probe action", async () => {
    ...
    expect(wrapper.text()).not.toContain(
      "admin.accounts.upstreamBilling.latest",
    );
    ...
    expect(
      wrapper
        .get('[data-testid="upstream-billing-probe"]')
        .attributes("aria-label"),
    ).toBe("admin.accounts.upstreamBilling.manualProbe");
  });
  ...
  it("uses retained failed data only while it is still fresh", async () => {
    ...
    expect(wrapper.get('[data-testid="upstream-billing-rate"]').text()).toBe(
      "admin.accounts.upstreamBilling.stale",
    );
  ...
  it("shows stale snapshot details, local next probe time, and the account probe state", async () => {
    ...
    expect(wrapper.getComponent(HelpTooltip).props("widthClass")).toBe(
      "w-max max-w-[calc(100vw-2rem)]",
    );
    await wrapper
      .get('[data-testid="upstream-billing-details"]')
      .trigger("mouseenter");
    await flushPromises();
    ...
    expect(tooltip.textContent).toContain(
      "admin.accounts.upstreamBilling.lastDetectedRate:0.9",
    );
    expect(tooltip.textContent).toContain(
      "admin.accounts.upstreamBilling.lastDetectedAt:",
    );
    expect(tooltip.textContent).toContain(
      "admin.accounts.upstreamBilling.elapsedSince:admin.accounts.upstreamBilling.hoursAgo:2",
    );
    expect(tooltip.textContent).toContain(
    ...
    expect(
      tooltip.querySelector('[data-testid="upstream-billing-probe-state"] span')
        ?.className,
    ...
    expect(
      tooltip.querySelector('[data-testid="upstream-billing-next-probe"]'),
    ).toBeNull();
    expect(
      tooltip.querySelector('[data-testid="upstream-billing-probe-state"] span')
        ?.className,
  ...
  it("stacks the global-off state below the account state and hides it when globally enabled", async () => {
    ...
    const tooltips = document.body.querySelectorAll('[role="tooltip"]');
    const tooltip = tooltips[tooltips.length - 1] as HTMLElement;
    const accountState = tooltip.querySelector(
      '[data-testid="upstream-billing-probe-state"]',
    );
    const globalState = tooltip.querySelector(
      '[data-testid="upstream-billing-global-probe-state"]',
    );
    expect(accountState?.querySelector("span")?.className).toContain(
      "text-emerald-400",
    );
    expect(globalState?.textContent).toContain(
      "admin.accounts.upstreamBilling.globalProbeState",
    );
    expect(globalState?.querySelector("span")?.className).toContain(
      "text-red-400",
    );
    expect(
      tooltip.querySelector('[data-testid="upstream-billing-next-probe"]'),
  ...
  it("fails neutral for malformed data and timestamps", async () => {
    ...
    await wrapper.setProps({
      account: malformedAccount({ billing_scope: "request" as "token" }),
    });
    expect(wrapper.get('[data-testid="upstream-billing-rate"]').text()).toBe(
      "-",
    );
    await wrapper.setProps({
      account: malformedAccount({}, { received_at: "not-a-time" }),
    });
    expect(wrapper.get('[data-testid="upstream-billing-rate"]').text()).toBe(
      "admin.accounts.upstreamBilling.stale",
    );
    await wrapper.setProps({
      account: malformedAccount({}, { received_at: "2026-07-13T00:31:00Z" }),
    });
    expect(wrapper.get('[data-testid="upstream-billing-rate"]').text()).toBe(
      "0.6x",
    );
    await wrapper.setProps({
      account: malformedAccount({}, { received_at: "2026-07-13T00:36:00Z" }),
    });
    expect(wrapper.get('[data-testid="upstream-billing-rate"]').text()).toBe(
      "admin.accounts.upstreamBilling.stale",
    );
    await wrapper.setProps({
      account: malformedAccount({}, { fresh_until: "2026-07-12T23:59:00Z" }),
    });
    expect(wrapper.get('[data-testid="upstream-billing-rate"]').text()).toBe(
      "admin.accounts.upstreamBilling.stale",
    );
