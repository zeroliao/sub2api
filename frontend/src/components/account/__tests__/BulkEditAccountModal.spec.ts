...
describe("BulkEditAccountModal", () => {
  ...
  it("antigravity 映射预设包含图片映射并过滤 OpenAI 预设", async () => {
    ...
    const mappingTab = wrapper
      .findAll("button")
      .find((btn) => btn.text().includes("admin.accounts.modelMapping"));
  ...
  it("所选含非 grok 平台时不展示快捷端点", async () => {
    ...
    expect(wrapper.findAll('[data-testid="grok-base-url-preset"]').length).toBe(
      0,
    );
  });
  ...
  it("OpenAI OAuth 批量编辑应提交 OAuth 专属 WS mode 字段（含 http_bridge）", async () => {
    ...
    await wrapper
      .get('[data-testid="bulk-edit-openai-ws-mode-select"]')
      .setValue("http_bridge");
    await wrapper.get("#bulk-edit-account-form").trigger("submit.prevent");
    await flushPromises();
  ...
  it("OpenAI API Key 批量编辑不显示 WS mode 入口", () => {
    ...
    expect(wrapper.find("#bulk-edit-openai-ws-mode-enabled").exists()).toBe(
      false,
    );
  });
  ...
  it("OpenAI OAuth 批量编辑应提交 codex_cli_only 字段", async () => {
    ...
    await wrapper
      .get("#bulk-edit-openai-codex-cli-only-enabled")
      .setValue(true);
    await wrapper
      .get("#bulk-edit-openai-codex-cli-only-toggle")
      .trigger("click");
    await wrapper.get("#bulk-edit-account-form").trigger("submit.prevent");
    await flushPromises();
  ...
  it("OpenAI OAuth 批量编辑应提交 codex_cli_only_allow_app_server 字段（需同时开启父开关）", async () => {
    ...
    await wrapper
      ...
      .trigger("click");
    await wrapper
      .get("#bulk-edit-openai-codex-app-server-enabled")
      .setValue(true);
    await wrapper
      .get("#bulk-edit-openai-codex-app-server-toggle")
      .trigger("click");
    await wrapper.get("#bulk-edit-account-form").trigger("submit.prevent");
    await flushPromises();
  ...
  it("OpenAI API Key 批量编辑应提交 API Key 专属 WS mode 字段", async () => {
    ...
    await wrapper
      .get("#bulk-edit-openai-apikey-ws-mode-enabled")
      .setValue(true);
    await wrapper
      .get('[data-testid="bulk-edit-openai-apikey-ws-mode-select"]')
      .setValue("ctx_pool");
    await wrapper.get("#bulk-edit-account-form").trigger("submit.prevent");
    await flushPromises();
  ...
  it("OpenAI API Key 批量编辑可统一开启上游倍率自动探测", async () => {
    ...
    await wrapper
      .get("#bulk-edit-upstream-billing-auto-probe-enabled")
      .setValue(true);
    await wrapper.get("#bulk-edit-account-form").trigger("submit.prevent");
    await flushPromises();
  ...
  it("OpenAI API Key 批量编辑可统一关闭上游倍率自动探测", async () => {
    ...
    await wrapper
      ...
      .setValue(true);
    await wrapper
      .get('[data-testid="bulk-edit-upstream-billing-auto-probe-select"]')
      .setValue("disabled");
    await wrapper.get("#bulk-edit-account-form").trigger("submit.prevent");
    await flushPromises();
  ...
  it("非 OpenAI API Key 目标不显示上游倍率自动探测批量开关", () => {
    ...
    expect(
      wrapper.find("#bulk-edit-upstream-billing-auto-probe-enabled").exists(),
    ).toBe(false);
  });
  ...
  it("筛选 OpenAI 账号批量编辑应提交 Compact 模式和专属模型映射", async () => {
    ...
    await wrapper
      .get('[data-testid="bulk-edit-openai-compact-mode-select"]')
      .setValue("force_on");
    await wrapper
      .get("#bulk-edit-openai-compact-model-mapping-enabled")
      .setValue(true);
    await wrapper
      .get('[data-testid="bulk-edit-openai-compact-model-mapping-add"]')
      .trigger("click");
