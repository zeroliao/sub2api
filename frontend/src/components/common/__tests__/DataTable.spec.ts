...
describe("DataTable", () => {
  ...
  it("renders paired sort arrows and highlights the active direction", async () => {
    ...
    expect(nameHeader.find('[data-test="custom-name-header"]').exists()).toBe(
      true,
    );
    expect(nameHeader.attributes("aria-sort")).toBe("ascending");
    expect(nameHeader.findAll("svg")).toHaveLength(2);
    expect(nameHeader.findAll("svg")[0].classes()).toContain(
      "text-primary-600",
    );
    ...
    expect(nameHeader.findAll("svg")[1].classes()).toContain(
      "text-primary-600",
    );
  });
  ...
  it("clears stale row and element caches when pagination replaces the row ID set", async () => {
    ...
    (instance as any).itemSizeCache = new Map(
      firstPageIDs.map((id) => [id, 156]),
    );
    ...
    expect(firstPageIDs.some((id) => instance.elementsCache.has(id))).toBe(
      false,
    );
  ...
  it("clears stale caches when equal-length pages replace rows without stable keys", async () => {
    ...
    const secondPage = Array.from({ length: 12 }, (_, i) => ({
      name: `Second ${i + 1}`,
    }));
    const wrapper = mount(DataTable, {
      props: {
  ...
  it("emits controlled current-page selection while preserving off-page keys", async () => {
    ...
    expect(rowCheckboxes.every((checkbox) => checkbox.element.checked)).toBe(
      true,
    );
