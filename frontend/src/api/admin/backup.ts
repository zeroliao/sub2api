...
export async function getS3Config(): Promise<BackupS3Config> {
  const { data } = await apiClient.get<BackupS3Config>(
...
export async function updateImageStorageConfig(
  ...
  const { data } = await apiClient.put<ImageStorageConfig>(
    "/admin/backups/image-storage",
    config,
  );
  return data;
...
export async function getSchedule(): Promise<BackupScheduleConfig> {
  const { data } = await apiClient.get<BackupScheduleConfig>(
...
export async function updateSchedule(
  ...
  const { data } = await apiClient.put<BackupScheduleConfig>(
    "/admin/backups/schedule",
    config,
  );
  return data;
...
export async function listBackups(): Promise<{ items: BackupRecord[] }> {
  const { data } = await apiClient.get<{ items: BackupRecord[] }>(
    "/admin/backups",
  );
  return data;
}
