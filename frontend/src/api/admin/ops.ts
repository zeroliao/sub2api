...
export function subscribeQPS(
  ...
  const connect = () => {
    ...
    const wsURL = wsBaseUrl
      ? new URL(
          `${window.location.protocol === "https:" ? "wss:" : "ws:"}//${wsBaseUrl}/api/v1/admin/ops/ws/qps`,
        )
      : new URL(
          buildGatewayUrl("/api/v1/admin/ops/ws/qps").replace(/^http/, "ws"),
        );
...
export async function getDashboardSnapshotV2(
  ...
  const { data } = await apiClient.get<OpsDashboardSnapshotV2Response>(
...
export async function getRuntimeLogConfig(): Promise<OpsRuntimeLogConfig> {
  const { data } = await apiClient.get<OpsRuntimeLogConfig>(
...
export async function updateRuntimeLogConfig(
  ...
  const { data } = await apiClient.put<OpsRuntimeLogConfig>(
    "/admin/ops/runtime/logging",
    config,
  );
  return data;
...
export async function resetRuntimeLogConfig(): Promise<OpsRuntimeLogConfig> {
  const { data } = await apiClient.post<OpsRuntimeLogConfig>(
...
export async function listSystemLogs(
  ...
  const { data } = await apiClient.get<OpsSystemLogListResponse>(
...
export async function getSystemLogSinkHealth(): Promise<OpsSystemLogSinkHealth> {
  const { data } = await apiClient.get<OpsSystemLogSinkHealth>(
