...
func (s *adminServiceImpl) DeleteAccount(ctx context.Context, id int64) error {
	...
	for _, shadow := range shadows {
		...
		if err := s.markAccountProxyBindingsDeleted(ctx, shadow); err != nil {
			logger.LegacyPrintf("service.admin", "failed to mark proxy bindings deleted for shadow account %d: %v", shadow.ID, err)
		}
	}
