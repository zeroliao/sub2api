...
`sub2api` 是应用源代码仓库，包含 Sub2API 服务的后端、前端、迁移文件和 Docker 镜像输入。生产部署由相邻的 `../zero007-sub2api-ops` 运维仓库管理。
...
- `../zero007-sub2api-ops/AGENTS.md`：当改动需要部署，或影响生产运维时阅读。
...
- 不要把生产密钥放进本仓库。
- 不要把 `sub2api/deploy/docker-compose*.yml` 当作生产事实来源。生产 compose 位于 `../zero007-sub2api-ops/deploy/docker-compose.yml`。
...
版本与分支管理以 `../zero007-sub2api-ops/docs/version-management.md` 为权威说明。
...
1. 从本仓库构建 Docker 镜像。
2. 将镜像推送到 registry。
3. 在 `../zero007-sub2api-ops/deploy/docker-compose.yml` 中更新到 immutable image digest。
4. 提交并推送两个仓库。
5. 从 `zero007-sub2api-ops` 执行 Git 驱动的运维部署。

如果新机器上缺少相邻运维仓库，在做生产部署工作前，先将 `git@github.com:zeroliao/zero007-sub2api-ops.git` clone 到本仓库旁边。
